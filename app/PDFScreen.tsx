...
export default function PDFScreen() {
  ...
  try {
    if (params.formData) {
      console.log('Received params.formData:', params.formData);
      formData = JSON.parse(params.formData as string) as FormData;
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.infoCard}>
        ...
        <Text style={styles.infoText}>• સંસ્થાનું નામ: {formData?.organizationName || 'ખાલી'}</Text>
        <Text style={styles.infoText}>• સંકુલનું નામ: {formData?.complexName || 'ખાલી'}</Text>
        <Text style={styles.infoText}>• ગામ/શહેર: {formData?.villageCity || 'ખાલી'}</Text>
      </View>
